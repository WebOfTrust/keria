...
def setup(
    ...
    agency = Agency(
        name=name, base=base, bran=bran, configFile=configFile, configDir=configDir
    )
    bootApp = falcon.App(
    ...
    if httpPort:
        ...
        specEnd = AgentSpecResource(
            app=app, title="KERIA Interactive Web Interface API"
        )
...
class Agency(doing.DoDoer):
    ...
    def __init__(
        self, name, bran, base="", configFile=None, configDir=None, adb=None, temp=False
    ):
        self.name = name
        self.base = base
        self.bran = bran
    ...
    def create(self, caid, salt=None):
        ...
        agentHab = agentHby.makeHab(
            f"agent-{caid}", ns="agent", transferable=True, delpre=caid
        )
        agentRgy = Regery(
...
class Agent(doing.DoDoer):
    ...
    def __init__(self, hby, rgy, agentHab, agency, caid, **opts):
        ...
        self.submitter = Submitter(hby=hby, submits=self.submits, witRec=self.witDoer)
        ...
        doers.extend(
            [
                ...
                ExchangeSender(
                    hby=hby, agentHab=agentHab, exc=self.exc, exchanges=self.exchanges
                ),
                Granter(
                    hby=hby,
                    rgy=rgy,
                ...
                ExchangeCueDoer(
                    seeker=self.exnseeker, cues=self.exc.cues, queries=self.queries
                ),
                self.submitter,
            ]
        )
                ...
                for recp in rec:
                    ...
                    try:
                        ...
                        for source, atc in sources:
                            sendArtifacts(
                                self.hby, self.rgy.reger, postman, source, recp
                            )
