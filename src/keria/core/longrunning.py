...
Typeage = namedtuple(
    "Tierage",
    "oobi witness delegation group query registry credential endrole challenge exchange submit "
    "done",
)
    ...
    def status(self, op):
        ...
        if op.type in (OpTypes.witness,):
            ...
            if "sn" not in op.metadata:
                raise kering.ValidationError(
                    f"invalid long running {op.type} operation, metadata missing 'sn' field"
                )
            ...
            if op.oid not in self.hby.kevers:
                ...
                if "sn" in op.metadata:
                    ...
                    if (
                        self.hby.db.findAnchoringSealEvent(op.oid, seal=anchor)
                        is not None
                    ):
                        operation.done = True
                        operation.response = asdict(kever.state())
