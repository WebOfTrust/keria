...
class RemoteKeeper(dbing.LMDBer):
    ...
    def __init__(self, headDirPath=None, perm=None, reopen=False, **kwa):
        ...
        super(RemoteKeeper, self).__init__(
    ...
    def reopen(self, **kwa):
        ...
        self.prxs = subing.CesrSuber(db=self, subkey="prxs.", klas=core.Cipher)
        self.nxts = subing.CesrSuber(db=self, subkey="nxts.", klas=core.Cipher)
        self.mhabs = subing.CesrSuber(db=self, subkey="mhabs.", klas=coring.Prefixer)
        self.pres = koming.Komer(
            db=self,
            subkey="pres.",
            schema=Prefix,
        )  # New Prefix
        self.sprms = koming.Komer(
            db=self,
            subkey="sprms.",
            schema=SaltyPrm,
        )  # New Salty Parameters
        self.sits = koming.Komer(
            db=self,
            subkey="sits.",
            schema=PreSit,
        )  # Prefix Situation
        self.pubs = koming.Komer(
            db=self,
            subkey="pubs.",
            schema=PubSet,
...
class SaltyManager:
    ...
    def incept(
        ...
        *,
        ...
        pp = Prefix(pidx=pidx, algo=Algos.salty)
    ...
    def rotate(
        ...
        if (pp := self.rb.pres.get(pre)) is None or pp.algo != Algos.salty:
            raise ValueError(
                "Attempt to rotate nonexistent or invalid pre={}.".format(pre)
            )
    ...
    def params(self, pre):
        if (pp := self.rb.pres.get(pre)) is None or pp.algo != Algos.salty:
            raise ValueError(
                "Attempt to load nonexistent or invalid pre={}.".format(pre)
            )

        if (pp := self.rb.sprms.get(pre)) is None:
            raise ValueError("Attempt to load nonexistent pre={}.".format(pre))

        prms = dict(salty=asdict(pp))
...
class RandyManager:
    ...
    def incept(self, pre, verfers, digers, prxs, nxts, transferable):
        ...
        if len(prxs) != len(verfers):
            raise ValueError(
                "If encrypted private keys are provided, must match verfers"
            )
    ...
    def rotate(self, pre, verfers, digers, prxs, nxts, transferable):
        if (pp := self.rb.pres.get(pre)) is None or pp.algo != Algos.randy:
            raise ValueError(
                "Attempt to rotate non-existant or invalid pre={}.".format(pre)
            )
...
class GroupManager:
    ...
    def rotate(self, pre, verfers, digers):
        if (pp := self.rb.pres.get(pre)) is None or pp.algo != Algos.group:
            raise ValueError(
                f"Attempt to rotate nonexistant or invalid  pre={pre}, algo={pp.algo}."
            )
