...
export class Indexer {
    ...
    constructor({raw = undefined, code = IdrDex.Ed25519_Sig, index = 0, ondex = undefined,
                qb64b = undefined, qb64 = undefined, qb2 = undefined}: IndexerArgs) {
        ...
        if (raw != undefined) {
            ...
            if (ondex != undefined && xizage.os != 0 && !(ondex >= 0 && ondex <= (64 ** os - 1))) {
    ...
    private _infil(): string {
        ...
        if (ondex != undefined && xizage.os != 0 && !(ondex >= 0 && ondex <= (64 ** xizage.os - 1))) {
            throw new Error(`Invalid ondex=${ondex} for os=${xizage.os} and code=${code}.`)
        }

        let both = `${code}${intToB64(index, ms)}${intToB64(ondex == undefined ? 0 : ondex, xizage.os)}`
        ...
        let full =  both + Base64.encode(Buffer.from(bytes)).slice(ps - xizage.ls)
