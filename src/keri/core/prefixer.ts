...
export class Prefixer extends Matter {
    ...
    _verify_ed25519N(ked: Dict<any>, pre: string, prefixed: boolean = false): boolean {
        try {
            let keys = ked["k"]
            if (keys.length != 1) {
                return false
            ...
            let next = 'n' in ked ? ked['n'] : []
            if (next.length > 0) { // must be empty
                return false
            }
        } catch (e) {
            return false
    ...
    _verify_ed25519(ked: Dict<any>, pre: string, prefixed: boolean = false): boolean {
        try {
            let keys = ked["k"]
            if (keys.length != 1) {
                return false
