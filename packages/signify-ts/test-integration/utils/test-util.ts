...
export async function assertOperations(
    ...
    for (const client of clients) {
        ...
        assert.partialDeepStrictEqual(operations, []);
    }
}
...
export async function createClient() {
    ...
    const client = new SignifyClient(env.url, passcode, Tier.low, env.bootUrl);
    await client.boot();
    await client.connect();
    return client;
}
...
export async function createIdentifier(
    ...
    if (!client.agent?.pre) {
        throw new Error('Client agent is not initialized');
    }

    const icp = await client.identifiers().create(name, args);
    await waitOperation(client, await icp.op());
    const hab = await client.identifiers().get(name);

    const rpy = await client
        .identifiers()
        .addEndRole(hab.name, 'agent', client.agent.pre);

    await waitOperation(client, await rpy.op());
