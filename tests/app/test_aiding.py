...
def test_identifier_collection_end(helpers):
    ...
    with helpers.openKeria() as (agency, agent, app, client), habbing.openHby(
        name="p1", temp=True, salt=salter.qb64
    ) as p1hby, habbing.openHby(name="p2", temp=True, salt=salter.qb64) as p2hby:
        end = aiding.IdentifierCollectionEnd()
...
class TestEscrower(doing.Doer):
    def __init__(self, kvy):
        ...
        super(TestEscrower, self).__init__()
...
def test_identifier_delegation_end(helpers):
    gatorname = "delegator"
    gatename = "delegate"
    ...
    #     gatename = "delegatee"
    #     op = helpers.createAid(client, gatename, salt, delpre=delpre)
    #     assert op["name"] == "delegation.EFt8G8gkCJ71e4amQaRUYss0BDK4pUpzKelEIr3yZ1D0"

    #     gatepre = op["metadata"]["pre"]
    #     gatesn = op["metadata"]["sn"]
    #     gatedig = op["metadata"]["pre"]
    #     anchor = dict(i=gatepre, s=gatesn, d=gatepre)
    #     serder = eventing.interact(pre=gatepre, dig=gatedig, data=[anchor])
    #     # sigs = (serder.raw));
    #     sbody = {
    #         "ixn": serder.ked,
    ...
    #     sres = client.simulate_post(
    #         path=f"/identifiers/{gatorname}/events?type=ixn", body=json.dumps(sbody)
    #     )
    #     assert sres.status_code == 200

    #     abody = {
    #         "approveDelegation": sbody["ixn"],
    #         "sigs": sbody["sigs"],
    #     }
    #     ares = client.simulate_post(
    #         path=f"/identifiers/{gatorname}?type=approveDelegation", body=json.dumps(abody)
    #     )
    #     assert ares.status_code == 202

    #     # Approve again
    #     badres = client.simulate_post(
    #         path=f"/identifiers/{gatorname}?type=approveDelegation", body=json.dumps(abody)
    #     )
    #     assert badres.status_code == 400


def test_identifier_extern_end(helpers):
    salt = b"0123456789abcdef"
    salter = core.Salter(raw=salt)
...
def test_challenge_ends(helpers):
    with helpers.openKeria() as (agency, agent, app, client):
        ...
        b = json.dumps(data).encode("utf-8")
        result = client.simulate_put(
            path=f"/challenges-verify/EFt8G8gkCJ71e4amQaRUYss0BDK4pUpzKelEIr3yZ1D0",
            body=b,
        )
...
def test_contact_ends(helpers):
    with helpers.openKeria() as (agency, agent, app, client), habbing.openHby(
        ...
        response = client.simulate_post(
            f"/contacts/E8AKUcbZyik8EdkOwXgnyAxO5mSIPJWGZ_o7zMhnNnjo/pal", body=b
        )
        ...
        response = client.simulate_get(
            f"/contacts/E8AKUcbZyik8EdkOwXgnyAxO5mSIPJWGZ_o7zMhnNnjo"
        )
        assert response.status == falcon.HTTP_404
        ...
        response = client.simulate_put(
            f"/contacts/E8AKUcbZyik8EdkOwXgnyAxO5mSIPJWGZ_o7zMhnNnjo", body=b
        )
        assert response.status == falcon.HTTP_404
        ...
        response = client.simulate_post(
            f"/contacts/E8AKUcbZyik8EdkOwXgnyAxO5mSIPJWGZ_o7zMhnNnjo/img",
            body=data,
            headers=headers,
        )
        assert response.status == falcon.HTTP_404
...
def test_identifier_resource_end(helpers):
    with helpers.openKeria() as (agency, agent, app, client), habbing.openHby(
        name="p1", temp=True
    ) as p1hby, habbing.openHby(name="p2", temp=True) as p2hby:
        end = aiding.IdentifierCollectionEnd()
        resend = aiding.IdentifierResourceEnd()
        app.add_route("/identifiers", end)
        app.add_route("/identifiers/{name}", resend)
        ...
        assert res.status_code == 404
        assert res.json == {
            "description": "bad is not a valid identifier name",
            "title": "404 Not Found",
        }
...
def test_approve_delegation(helpers):
    ...
    with habbing.openHby(name="keria", salt=salter.qb64, temp=True, cf=cf) as hby:
        ...
        sres = client.simulate_put(
            path=f"/agent/{controllerAID}?type=ixn", body=json.dumps(sbody)
        )
        assert sres.status_code == 204

        fbody = {}
        fres = client.simulate_put(
            path=f"/agent/{controllerAID}?type=ixn", body=json.dumps(fbody)
        )
        assert fres.status_code == 400

        fbody = {"sigs": "fake_sigs"}
        fres = client.simulate_put(
            path=f"/agent/{controllerAID}?type=ixn", body=json.dumps(fbody)
        )
        assert fres.status_code == 400
