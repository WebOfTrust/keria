...
describe('int to b64 and back', () => {
    it('should encode and decode stuff', async () => {
        ...
        Counter.Sizes.forEach((val, _) => {
            assert.equal((val.hs > 0 && val.ss > 0 && val.hs + val.ss == val.fs && !(val.fs % 4)), true)
        })
        ...
        counter = new Counter({code: CtrDex.BigAttachedMaterialQuadlets, count: count})
        ...
        counter = new Counter({code: CtrDex.KERIProtocolStack, count: verint})
