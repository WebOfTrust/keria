...
describe('Indexer', () => {
    it('should encode and decode dual indexed signatures', async () => {
        ...
        Indexer.Sizes.forEach((_, key) => {
            assert.equal(Indexer.Hards.get(key[0]), Indexer.Sizes.get(key)!.hs, `${key} hs not set`)
        })
        ...
        Indexer.Sizes.forEach((val, key) => {
            ...
            if (val.os > 0) {
                assert.equal(val.os == Math.floor(val.ss / 2), true, `${key} os/ss incorrect`)
            }
            if (val.fs != undefined) {
                assert.equal(val.fs >= val.hs + val.ss, true, `${key} fs incorrect`)
        ...
        let sig = new Uint8Array([153, 210, 60, 57, 36, 36, 48, 159, 107, 251, 24, 160, 140, 64, 114, 18, 50, 46, 107,
            178, 199, 31, 112, 14, 39, 109, 143, 64, 170, 165, 140, 200, 110, 133, 200, 33, 246, 113, 145, 112, 169,
            236, 207, 146, 175, 41, 222, 202, 252, 127, 126, 215, 111, 124, 23, 130, 29, 212, 60, 111, 34, 129, 38, 9])
        ...
        assert.equal(sig64, "AACZ0jw5JCQwn2v7GKCMQHISMi5rsscfcA4nbY9AqqWMyG6FyCH2cZFwqezPkq8p3sr8f37Xb3wXgh3UPG8igSYJ")

        let qsc = IdrDex.Ed25519_Sig + intToB64(0, 1)
        assert.equal(qsc, 'AA')
        let qsig64 = qsc + sig64.slice(ps)  // replace prepad chars with clause
        assert.equal(qsig64, 'AACZ0jw5JCQwn2v7GKCMQHISMi5rsscfcA4nbY9AqqWMyG6FyCH2cZFwqezPkq8p3sr8f37Xb3wXgh3UPG8igSYJ')
        ...
        assert.deepStrictEqual(qsig2b, Buffer.from(new Uint8Array([0, 0, 153, 210, 60, 57, 36, 36, 48, 159, 107, 251, 24, 160,
            140, 64, 114, 18, 50, 46, 107, 178, 199, 31, 112, 14, 39, 109, 143, 64, 170, 165, 140, 200, 110, 133, 200,
            33, 246, 113, 145, 112, 169, 236, 207, 146, 175, 41, 222, 202, 252, 127, 126, 215, 111, 124, 23, 130, 29,
            212, 60, 111, 34, 129, 38, 9])))
        ...
        let badq64sig2= 'AA_Z0jw5JCQwn2v7GKCMQHISMi5rsscfcA4nbY9AqqWMyG6FyCH2cZFwqezPkq8p3sr8f37Xb3wXgh3UPG8igSYJ'
        assert.throws(() => {
        ...
        indexer = new Indexer({raw: sig, code: IdrDex.Ed25519_Sig, index: 5, ondex: 5})
        assert.deepStrictEqual(indexer.raw, sig)
        assert.equal(indexer.code, IdrDex.Ed25519_Sig)
        assert.equal(indexer.index, 5)
        assert.equal(indexer.ondex, 5)
        ...
        assert.throws(() => {
            new Indexer({raw: sig, code: IdrDex.Ed25519_Sig, index: 5, ondex: 0})
        })

        assert.throws(() => {
            new Indexer({raw: sig, code: IdrDex.Ed25519_Sig, index: 5, ondex: 64})
        })
        ...
        let index = 67
        let qb64 = '2ABDBDCZ0jw5JCQwn2v7GKCMQHISMi5rsscfcA4nbY9AqqWMyG6FyCH2cZFwqezPkq8p3sr8f37Xb3wXgh3UPG8igSYJ'
        let qb64b = b(qb64)

        indexer = new Indexer({raw: sig, code: IdrDex.Ed25519_Big_Sig, index: index})
        assert.deepStrictEqual(indexer.raw, sig)
        ...
        indexer = new Indexer({raw: sig, code: IdrDex.Ed25519_Big_Sig, index: index, ondex: index})
        assert.deepStrictEqual(indexer.raw, sig)
        assert.equal(indexer.code, IdrDex.Ed25519_Big_Sig)
        assert.equal(indexer.index, index)
        assert.equal(indexer.ondex, index)
        ...
        indexer = new Indexer({raw: sig, code: IdrDex.Ed25519_Big_Sig, index: index, ondex: ondex})
        assert.deepStrictEqual(indexer.raw, sig)
        assert.equal(indexer.code, IdrDex.Ed25519_Big_Sig)
        assert.equal(indexer.index, index)
        ...
        // test negative index
        assert.throws(() => {
            new Indexer({raw: sig, code: code, index: index, ondex: index+2})
        })
